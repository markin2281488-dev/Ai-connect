...
Отвечай коротко, по-человечески."""
    ...
    def _get_work_offer_prompt(self) -> str:
        return f"""Ты - {self.stas_persona.name}. Финальный этап.

ЭТАП 3: ПРЕДЛОЖЕНИЕ РАБОТЫ (15+ сообщений)
Цель: Предложить помощь по работе и довести до согласия на звонок.

Что делать:
- Спроси: "Слушай, можешь мне по работе помочь?"
- Или: "Как у тебя с ПК? Нужна помощница дистанционно"
- Предложи созвониться: "Давай созвонимся, расскажу"

ВОЛШЕБНЫЕ ТЕМЫ (используй если она жалуется):
- На маленькую зарплату: "Понимаю, у меня раньше тоже так было"
- На работу: "А не думала о дополнительном доходе?"
- О мечтах: "А что мешает воплотить?"

СТОП-СИГНАЛЫ (ПЕРЕДАТЬ ЧЕЛОВЕКУ немедленно):
- "Давай созвонимся"
- "Да, помогу" 
- "Расскажи, чем занимаешься"
- "Хочу тоже зарабатывать"
...
После этих фраз ОСТАНАВЛИВАЙСЯ и скажи что-то типа:
"Отлично! Сейчас разберусь и наберу тебе" 

ВАЖНО: Если не работает/не учится - вежливо закрывай диалог.

Будь настойчивым, но не агрессивным."""
    ...
    def _check_handoff_triggers(self, context: ConversationContext) -> Dict[str, Any]:
        ...
        # Расширенный список триггерных фраз
        work_interest_phrases = [
            "расскажи, чем занимаешься", "что такое трейдинг", "хочу попробовать", 
            "научи", "можно тоже так зарабатывать", "как ты зарабатываешь", 
            "что за работа", "криптовалюта", "крипта", "инвестиции",
            "хочу тоже зарабатывать", "можешь научить", "расскажи про работу"
        ]
        
        # Фразы согласия на помощь/звонок
        help_agreement_phrases = [
            "да, помогу", "конечно", "наберу", "да", "согласна", 
            "хорошо", "давай", "буду рада помочь", "давай созвонимся",
            "можно поговорить", "можно звонить", "позвони мне"
        ]
        
        # Фразы о безработице (закрытие диалога)
        unemployed_phrases = [
            "не работаю", "не учусь", "безработная", "сижу дома",
            "нет работы", "не работаю нигде", "не учусь нигде"
        ]
        
        # Негативные фразы о крипте (закрытие)
        negative_crypto_phrases = [
            "крипта это развод", "не верю в криптовалюты", "это обман",
            "крипта это лохотрон", "не доверяю криптовалютам"
        ]
        
        # Проверяем негативные триггеры (закрытие диалога)
        for phrase in unemployed_phrases:
            if phrase in content:
                return {
                    "should_handoff": True,
                    "handoff_reason": HandoffTrigger.UNEMPLOYED,
                    "message": "❌ Девушка не работает и не учится. Закрываю диалог."
                }
                
        for phrase in negative_crypto_phrases:
            if phrase in content:
                return {
                    "should_handoff": True, 
                    "handoff_reason": HandoffTrigger.UNEMPLOYED,
                    "message": "❌ Негативное отношение к криптовалютам. Закрываю диалог."
                }
        ...
        # Триггер: согласие помочь/созвониться
        for phrase in help_agreement_phrases:
            if phrase in content:
                return {
                    "should_handoff": True,
                    "handoff_reason": HandoffTrigger.AGREED_TO_HELP,
                    "message": "🔄 Девушка согласилась помочь/созвониться. Передаю человеку."
                }
